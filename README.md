...
| Messaging | Amazon MQ for RabbitMQ (`mq.t3.micro`, single instance, AMQPS/TLS) |
...
- `aws --version` (≥2.x), `terraform version` (≥1.5), `docker --version`
- `mysql` client on PATH — `brew install mysql-client` on macOS (follow caveats to add to PATH)
...
| Amazon MQ `mq.t3.micro`, single instance | ~$14.40 | ~$0.50 |
...
> turns into a $80 surprise. Set the AWS Budget alert.
