...
func TestDispatch_Success_Acks(t *testing.T) {
	ack := &fakeAck{}
	rep := &mockRepublisher{}
	dispatchMsg(context.Background(), newDelivery("payment.processed", nil, ack), zap.NewNop(), rep,
		"order.events", 5, func(ctx context.Context, m amqp.Delivery) error { return nil })
...
func TestDispatch_TransientUnderCap_RepublishesToRetryAndAcks(t *testing.T) {
	ack := &fakeAck{}
	rep := &mockRepublisher{}
	dispatchMsg(context.Background(), newDelivery("payment.processed", amqp.Table{amqpretry.HeaderRetryCount: int32(1)}, ack),
		zap.NewNop(), rep, "order.events", 5,
	...
	if assert.Len(t, rep.calls, 1) {
		...
		assert.Equal(t, "payment.processed", rep.calls[0].routingKey)
...
func TestDispatch_TransientAtCap_RepublishesToDLQ(t *testing.T) {
	ack := &fakeAck{}
	rep := &mockRepublisher{}
	dispatchMsg(context.Background(), newDelivery("payment.processed", amqp.Table{amqpretry.HeaderRetryCount: int32(5)}, ack),
		zap.NewNop(), rep, "order.events", 5,
		func(ctx context.Context, m amqp.Delivery) error { return errors.New("db blip") })
	assert.True(t, ack.acked)
	if assert.Len(t, rep.calls, 1) {
		assert.Equal(t, "", rep.calls[0].exchange)
		assert.Equal(t, "order.events.dlq", rep.calls[0].routingKey)
	}
}
...
func TestDispatch_Permanent_RepublishesToDLQ(t *testing.T) {
	ack := &fakeAck{}
	rep := &mockRepublisher{}
	dispatchMsg(context.Background(), newDelivery("payment.processed", nil, ack),
		zap.NewNop(), rep, "order.events", 5,
...
func TestDispatch_DLQRepublishFails_NacksWithoutRequeue(t *testing.T) {
	...
	dispatchMsg(context.Background(),
		newDelivery("payment.processed", amqp.Table{amqpretry.HeaderRetryCount: int32(5)}, ack),
		zap.NewNop(), rep, "order.events", 5,
		func(ctx context.Context, m amqp.Delivery) error { return errors.New("db blip") })
	assert.True(t, ack.nacked)
