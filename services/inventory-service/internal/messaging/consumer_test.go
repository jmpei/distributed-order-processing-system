...
func TestDispatch_Success_Acks(t *testing.T) {
	ack := &fakeAck{}
	rep := &mockRepublisher{}
	dispatchMsg(context.Background(), newDelivery("inventory.reserve", nil, ack), zap.NewNop(), rep,
		"inventory.commands", 5, func(ctx context.Context, m amqp.Delivery) error { return nil })
...
func TestDispatch_TransientUnderCap_RepublishesToRetryAndAcks(t *testing.T) {
	ack := &fakeAck{}
	rep := &mockRepublisher{}
	dispatchMsg(context.Background(), newDelivery("inventory.reserve", amqp.Table{amqpretry.HeaderRetryCount: int32(1)}, ack),
	...
	if assert.Len(t, rep.calls, 1) {
		...
		assert.Equal(t, "inventory.reserve", rep.calls[0].routingKey)
...
func TestDispatch_TransientAtCap_RepublishesToDLQ(t *testing.T) {
	ack := &fakeAck{}
	rep := &mockRepublisher{}
	dispatchMsg(context.Background(), newDelivery("inventory.reserve", amqp.Table{amqpretry.HeaderRetryCount: int32(5)}, ack),
		zap.NewNop(), rep, "inventory.commands", 5,
		func(ctx context.Context, m amqp.Delivery) error { return errors.New("db blip") })
	assert.True(t, ack.acked)
	if assert.Len(t, rep.calls, 1) {
		assert.Equal(t, "", rep.calls[0].exchange)
		assert.Equal(t, "inventory.commands.dlq", rep.calls[0].routingKey)
	}
}
...
func TestDispatch_Permanent_RepublishesToDLQ(t *testing.T) {
	ack := &fakeAck{}
	rep := &mockRepublisher{}
	dispatchMsg(context.Background(), newDelivery("inventory.reserve", nil, ack),
		zap.NewNop(), rep, "inventory.commands", 5,
